...
var createHSM2Slot = function(params, callback) {
    openssl.listPKCS11Slots({modulePath: '/usr/lib/softhsm/libsofthsm2.so'}, function(err, slotout, cmd) {
        //console.log(cmd);
        if(err) {
            callback(err, false);
        } else {
            ...
            //console.log(label)
            let cmd = ['--module /usr/lib/softhsm/libsofthsm2.so --slot ' + originalslotid + ' --init-token --label ' + label + ' --so-pin ' + params.sopin];
            //let cmd = ['--module /usr/lib/softhsm/libsofthsm2.so --slot ' + originalslotid + ' --init-token --label \'' + params.label + '\' --so-pin ' + params.sopin];
                ...
                if(err) {
                    ...
                    openssl.listPKCS11Slots({modulePath: '/usr/lib/softhsm/libsofthsm2.so'}, function(err, slotout, cmd) {
                        ...
                        if(err) {
                            ...
                            let cmd = ['--module /usr/lib/softhsm/libsofthsm2.so --slot ' + newslotid + ' --login --login-type so --so-pin ' + params.sopin + ' --init-pin --pin ' + params.pin];
                            //console.log('look here');
                            //console.log(cmd);
                            pkcs11tool.run(cmd.join(' '), function(err, out) {
                                //console.log(out);
...
var abstractImport = function(params, callback) {
    ...
    if(params.module.indexOf('libykcs') >= 0) {
        ...
        openssl.generateCSR(csroptions, params.key, 'test', function(err, csr, cmd) {
            if(err) {
                callback(err, false);
            } else {
                openssl.selfSignCSR(csr, csroptions, params.key, 'test', function(err, crt, cmd) {
                    if(err) {
                        callback(err, false);
                    } else {
                        openssl.createPKCS12(crt, params.key, 'test', 'temppassword', false, function(err, pfx, command) {
                            if(err) {
                                //console.log(err);
                                //console.log(command);
                            } else {
                                    ...
                                    if (err) {
                                        ...
                                        fs.writeFile(pfxpath, pfx, function() {
                                            if(err) {
                                                ...
                                                let cmd = ['-aimport-certificate -aimport-key -k' + params.pin + ' -s' + mapping[params.objectid] + ' -i' + pfxpath + ' -ptemppassword -KPKCS12'];
                                                console.log(cmd);
                    ...
                    if (err) {
                        ...
                        fs.writeFile(derpath, data, function() {
                            if(err) {
                                ...
                                let cmd = ['--label ' + params.label.split(' ').join('\\ ') + ' --module ' + params.module + ' ' + login + ' --slot ' + params.slotid + ' --id ' + params.objectid + ' --write-object ' + derpath + ' --type privkey'];
                                //console.log(cmd.join(' '));
                                //console.log(data);
                                pkcs11tool.run(cmd.join(' '), function(err, out) {
                                    //console.log(out);
                                            ...
                                            if(err) {
                                                ...
                                                tmp.file(function _tempFileCreated(err, pubkeypath, fd, cleanupCallback) {
                                                    if (err) {
                                                        ...
                                                        fs.writeFile(pubkeypath, out.stdout, function() {
                                                            if(err) {
                                                                cleanupCallback();
                                                                callback(err, false);
                                                            } else {
