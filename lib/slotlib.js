                ...
                if(type) {
                    openssl.readPKCS11Object({slotid: slot.hexid, objectid: slot.objects[ids[idindex]][types[typeindex]][objects[objectindex]].ID, type: type, modulePath: slot.modulePath}, function(err, slotout, cmd) {
                        ...
                        if(err) {
                            ...
                            if(type=='cert') {
                                openssl2.x509.parse({cert: slotout}, function(err, attrs, cmd) {
                                    if(err) {
                                        callback(err, false);
                                    } else {
...
var generateKeyPair = function(params, callback) {
    ...
    pkcs11tool.run(cmd.join(' '), function(err, out) {
        ...
        if(err) {
            ...
            openssl.selfSignCSRv2({options: csroptions, pkcs11: {pin: params.signpin, serial: params.serial, slotid: params.objectid}} , function(err, crt, cmd) {
                if(err) {
                    callback(err, false);
                } else {
                    //console.log(crt);
...
var getCertInfo = function(params, callback) {
    //console.log(params);
    openssl2.x509.parse({cert: params.template}, function(err, cert, cmd) {
        if(err) {
            openssl2.csr.parse({csr: params.template}, function(err, cert, cmd) {
                if(err) {
                    callback(err, false);
                } else {
