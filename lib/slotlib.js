...
var checkGoogleKMS = function(callback) {
    ...
    fs.stat(config.GOOGLE_APPLICATION_CREDENTIALS, function(err, stats) {
        if(err) {
            // might be using a different method of authentication
        } else {
            process.env['GOOGLE_APPLICATION_CREDENTIALS'] = config.GOOGLE_APPLICATION_CREDENTIALS;
            console.log('Google Application Credentials found, loading Google KMS PKCS11 module');
        }
        // if pkcs11-kms.yml file exists and 
        fs.stat(__dirname + '/../state/pkcs11-kms.yml', function(err, stats) {
            if(err) {
                // callback(err, false);
                callback(false, false);
            } else {
                fs.cp(__dirname + '/../state/pkcs11-kms.yml', __dirname + '/../tmp/pkcs11-kms.yml', function (err) {
                    if (err) {
                        callback(err, false);
                    } else {
                        fs.chmod(__dirname + '/../tmp/pkcs11-kms.yml', 0o600, (err) => {
                            if (err) {
                                callback(err, false);
                            } else {
                                callback(false, true);
...
var getModules = function(callback) {
    ...
    checkGoogleKMS(function(err, result) {
        if(err) {
            // callback(err, false);
        } else {
            if(result) {
