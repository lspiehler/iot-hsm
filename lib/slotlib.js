                ...
                if(type) {
                    openssl.readPKCS11Object({slotid: slot.hexid, objectid: slot.objects[ids[idindex]][types[typeindex]][objects[objectindex]].ID, type: type, modulePath: slot.modulePath}, function(err, slotout, cmd) {
                        ...
                        if(err) {
                            ...
                            if(type=='cert') {
                                openssl2.x509.parse({cert: slotout}, function(err, attrs) {
                                    if(err) {
                                        callback(err, false);
                                    } else {
                                        ...
                                        attrs.data.distinguishedName = openssl.getDistinguishedName(attrs.data.subject)
            ...
            openssl.listPKCS11Objects({modulePath: slots[index].modulePath, slotid: slots[index].hexid}, function(err, objectout, cmd) {
                ...
                if(err) {
                    ...
                    for(let i = 0; i <= objectout.length - 1; i++) {
                        if(objectout[i].type == 'Certificate Object' && objectout[i].label.indexOf('X.509 Certificate for PIV Attestation') != 0) {
            ...
            if(err) {
                ...
                openssl.generateCSR(csroptions, params.key, params.keypass, function(err, csr, cmd) {
                    if(err) {
                        ...
                        openssl2.x509.selfSignCSR({csr: csr, options: csroptions, key: params.key, password: params.keypass, modulePath: params.module}, function(err, crt) {
                            if(err) {
                                callback(err, false);
                            } else {
                                openssl.createPKCS12(crt.data, params.key, params.keypass, randomstring, false, function(err, pfx, command) {
                                    if(err) {
                                        //console.log(err);
                                        //console.log(command);
                                    } else {
...
var generateCSR = function(params, callback) {
    ...
    openssl.getCertInfo(params.template, function(err, cert, cmd) {
        if(err) {
            ...
            openssl2.x509.generateCSR({options: csroptions, pkcs11: {pin: params.pin, serial: params.serial, objectid: params.objectid, modulePath: params.module}} , function(err, csr) {
                if(err) {
                    callback(err, false);
                } else {
                    //console.log(csr);
