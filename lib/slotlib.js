...
var generateSelfSigned = function(params, callback) {
    ...
    getCertInfo(params, function(err, cert) {
        if(err) {
            ...
            if(uri['objectid'] && uri['objectid'].length > 100) {
                uri['label'] = params.objectid;
            } else {
                uri['objectid'] = params.objectid;
            }
            openssl2.x509.selfSignCSR({
                    options: csroptions,
                    pkcs11: {
                        uri: openssl2.common.encodePKCS11URI(uri),
                        pin: params.pin,
                        modulePath: params.module
                    }
                }, function(err, crt) {
        ...
        if(err) {
            ...
            fs.stat(config.KMS_PKCS11_CONFIG, function(err, stats) {
                if(err) {
                    ...
                    modules.push('/usr/local/lib/kms/libkmsp11.so');
                }
                console.log(modules);
