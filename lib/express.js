...
app.post('/google-auth-upload', (req, res) => {
    ...
    upload(req, res, (err) => {
        ...
        // copy uploaded file to the state directory
        fs.cp(__dirname + '/../' + req.files[0].path, __dirname + '/../state/' + config.GOOGLE_APPLICATION_CREDENTIALS.split('/').pop(), function (err) {
            if (err) {
            ...
            fs.unlink(__dirname + '/../' + req.files[0].path, function (err) {
                if (err) {
                    console.log(err);
                }
            });
