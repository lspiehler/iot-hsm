...
var signMessage = function(cert, message, callback) {
    //console.log(cert);
    smimelib.x509.getCert(cert, function(err, newcert) {
        if(err) {
            callback(err, false);
        } else {
            //console.log(cert);
            //cert.channel.cert = newcert;
            smimelib.sign({cert: newcert, key: cert.channel.key, data: message}, function(err, smimesign) {
                if(err) {
                    callback(err, false);
                } else {
                    //console.log(smimesign);
                    callback(false, smimesign);
                }
            });
        }
    });
    ...
    verifyMessage(message.message.smime, function(err, resp) {
        if(err) {
            ...
            if(message.message.type=='solicit signature') {
                ...
                signMessage(ca, JSON.stringify(signmsg), function(err, smime) {
                    if(err) {
                        console.log(err);
                    } else {
                        //console.log(smime);
                ...
                let cert = certs[message.message.caindex];
                smimelib.decrypt({ data: resp.data, cert: cert.channel.cert, key: cert.channel.key }, function(err, decrypt) {
                    if(err) {
                        console.log(err);
                        callback(err, false);
                    } else {
                                    ...
                                    if(ca) {
                                        slotlib.signCSR({ publiccert: ca.base64, slotid: ca['token hexid'], csr: vmsg.request.csr, options: vmsg.request.options, module: ca.module, serial: ca['token serial'], pin: ca['token pin'], objectid: ca['ID']}, function(err, signedcert) {
                                            if(err) {
                                                ...
                                                let input = Buffer.from(JSON.stringify(signmsg), 'utf8');
                                                //console.log(input);
                                                zlib.deflate(input, function(err, buf) {
                                                    if(err) {
                                                        console.log('Failed to deflate message');
                                                    } else {
                                                        signMessage(ca, buf, function(err, smime) {
                                                            if(err) {
                                                                console.log(err);
                                                            } else {
                                                                //console.log(smime);
                                                ...
                                                zlib.deflate(input, function(err, buf) {
                                                    if(err) {
                                                        ...
                                                        smimelib.encrypt({cert: resp.certs[0], data: buf}, function(err, smimeenc) {
                                                            if(err) {
                                                                ...
                                                                signMessage(ca, smimeenc, function(err, smime) {
                                                                    if(err) {
                                                                        console.log(err);
                                                                    } else {
                                                                        //console.log(smime);
