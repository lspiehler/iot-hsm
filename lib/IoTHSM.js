                                        ...
                                        if(err) {
                                            ...
                                            slotlib.signCRL({ publiccert: ca.base64, slotid: ca['token hexid'], database: vmsg.request.database, days: vmsg.request.days, module: ca.module, serial: ca['token serial'], pin: userpin, objectid: ca['ID']}, function(crlerr, signedcrl) {
                                                if(crlerr) {
                                                    ...
                                                    encryptMessage({enccert: params.verified.certs[0], signed: JSON.stringify(signed)}, function(err, smimeenc) {
                                                        if(err) {
                                                            signed.message = err;
                                                            sendEncryptedMessage({publisher: params.message.publisher, channel: params.message.channel, signcert: ca, enccert: params.verified.certs[0], signed: JSON.stringify(signed), unsigned: unsigned}, function(err, resp) {
                                                                if(err) {
...
module.exports = {
    ...
    getSlots: function() {
        ...
        for(let i = 0; i <= certs.length - 1; i++) {
            ...
            iot.hsm[certs[i]['token serial']][certs[i].ID] = { connected: false }
            if(certs[i].channel.id) {
                iot.hsm[certs[i]['token serial']][certs[i].ID].connected = true;
                iot.connected++;
            } else {
                iot.hsm[certs[i]['token serial']][certs[i].ID].message = certs[i].channel.message
            }
        }
        return iot;
    }
