        ...
        if(err) {
            ...
            smimelib.encrypt({cert: params.enccert, data: buf}, function(err, smimeenc) {
                if(err) {
                    ...
                    sendSignedMessage({channel: params.channel, cert: params.signcert, signed: smimeenc, unsigned: params.unsigned}, function(err, resp) {
                        if(err) {
                            callback(err, false);
                        } else {
                            callback(false, resp);
    ...
    verifyMessage(message.message.smime, function(err, resp) {
        if(err) {
            ...
            if(message.message.type=='solicit signature') {
                ...
                sendSignedMessage({channel: message.channel, cert: ca, signed: JSON.stringify(signed), unsigned: unsigned}, function(err, resp) {
                    if(err) {
                        console.log(resp);
                    } else {
                        console.log(resp)
                                    ...
                                    if(vmsg.uuid==pubnub.getUUID()) {
                                        ...
                                        slotlib.signCSR({ publiccert: ca.base64, slotid: ca['token hexid'], csr: vmsg.request.csr, options: vmsg.request.options, module: ca.module, serial: ca['token serial'], pin: ca['token pin'], objectid: ca['ID']}, function(err, signedcert) {
                                            if(err) {
                                                ...
                                                sendEncryptedMessage({channel: message.channel, signcert: ca, enccert: resp.certs[0], signed: JSON.stringify(signed), unsigned: unsigned}, function(err, resp) {
                                                    if(err) {
                                                        //console.log(err);
                                                        console.log(resp);
                                                    } else {
