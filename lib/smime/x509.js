        ...
        if(err) {
            ...
            pinlib.getPins({serial: params.cert['token serial']},function(err, pins) {
                if(err) {
                    ...
                    //console.log(userpin);
                    slotlib.signCSR({ publiccert: params.cert.base64, slotid: params.cert['token hexid'], csr: csr, options: csroptions, module: params.cert.module, serial: params.cert['token serial'], pin: userpin, objectid: params.cert['ID']}, function(err, signedcert) {
                        if(err) {
                            callback(err, false);
                        } else {
