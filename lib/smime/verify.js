...
var verify = function(params, callback) {
    //console.log(params);
    if(typeof(params.smime)=='object') {
        callback('Invalid SMIME data', false);
    } else {
                            ...
                            if(err) {
                                ...
                                customOpenSSLCommand.run({cmd: command, stdin: out.stdout}, function(err, out) {
                                    ...
                                    if(err) {
                                        ...
                                        //console.log(out.stdout);
                                        let command = 'cms -verify -inform SMIME -binary -CAfile ' + path;
                                        customOpenSSLCommand.run({cmd: command, stdin: params.smime}, function(err, out) {
                                            cleanupCallback();
                                            //console.log(out);
                                            if(err) {
                                                ...
                                                //console.log(out);
                                                callback(false, {certs: certs, data: out.stdout});
                                            }
                                        });
                                    }
                                });
